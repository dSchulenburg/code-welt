...
const name = mlang({ de: 'Über Holz', en: 'About wood', uk: 'Про дерево', ar: 'عن الخشب', it: 'Sul legno' });
...
`;
  ...
  const checks = [
    ...
    ['Name es (kein eigener Block → other=de)', raw.NAME_es === 'Über Holz'],
  ...
  await callTool('moodle_delete_course', { courseId }).catch((e) => console.error('Aufräumen fehlgeschlagen, Kurs ' + courseId + ':', e.message));
}
