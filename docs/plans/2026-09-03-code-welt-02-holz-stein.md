...
- [ ] **Step 1: Failing tests** — `content.test.js`: jede Station hat `iframeHeight` ≥ 800; letzte Station jeder nicht-leeren Etappe hat `bossCheck` und i18n `bossCheck.title/task`; jede Etappe hat `badge` und i18n `badge.name/description`; story-`mood`, wenn gesetzt, aus der erlaubten Menge. `course-def.test.js`: Holz enthält ein `assignment`-Item `boss-holz` mit `{mlang uk}` im Namen; Abschnitt 1 enthält `page`-Items in der Reihenfolge der Markdown-Dateien und ein `folder`-Item `weltdateien`; iframe für `ar` hat `height="1540"` bei `iframeHeight: 1400`.
